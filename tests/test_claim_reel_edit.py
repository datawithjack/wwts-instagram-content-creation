...
def test_pro_reel_claims_on_the_profile_then_joins_the_board():
    ...
    assert [i[1] if i[0] == "card" else i[-1] for i in plan] == [
        "pro_hook", "fantasy", "pro_who", "pro_claim", "profile", "pro_join", "board",
...
def test_a_missing_take_drops_its_slot_and_keeps_the_rest():
    ...
    assert [i[-1] if i[0] == "footage" else i[1] for i in plan] == [
        "coach_hook", "fantasy", "coach_who", "coach_claim", "coach_join", "board",
        "coach_why", "coach_soon", "coach_cta",
    ]
...
def test_podium_reel_calls_the_podium_then_hands_over_to_the_team():
    plan = plan_reel("podium", {"podium": {"podium_start": 3.0, "podium_end": 15.0,
                                           "heat_start": 15.0, "heat_end": 18.0}})
    assert plan == [
        ("card", "podium_hook"),
